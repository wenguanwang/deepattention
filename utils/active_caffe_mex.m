...
% --------------------------------------------------------
    ...
    cur_dir = pwd;
    caffe_dir = fullfile(fileparts(fileparts(mfilename('fullpath'))), 'external', 'caffe', 'matlab', caffe_version);
    
    if ~exist(caffe_dir, 'dir')
        warning('Specified caffe folder (%s) is not exist, change to default one (%s)', ...
            caffe_dir, fullfile(fileparts(fileparts(mfilename('fullpath'))), 'external', 'caffe', 'matlab'));
        caffe_dir = fullfile(fileparts(fileparts(mfilename('fullpath'))), 'external', 'caffe', 'matlab');
    end
